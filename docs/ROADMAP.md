...
2. **Kata sandbox adoption**: Make the migrated QEMU process appear as a regular Kata pod on the destination, visible to `kubectl`, manageable by Deployments, monitored by kubelet.
   ...
   **Paths forward**:
   ...
   - **Approach E (process re-parenting): in progress.** Step 1 (cgroup re-parent so the migrated QEMU survives the dest job's container exit) is **landed**: `internal/migration/dest.go::surviveContainerExit` writes the QEMU pid + its KVM helper kernel threads (`kvm-nx-lpage-recovery-<qemupid>`) into `/sys/fs/cgroup/katamaran-adopted/<sandbox-id>/cgroup.procs`. Live-verified end-to-end: 9m20s hang → 35s clean success. The QEMU now outlives the dest job container and is reachable on its QMP socket from any process on the dest node. Step 2 (custom containerd v2 shim that adopts the surviving QEMU into a new pod's lifecycle) is **implemented but not yet verified against a real migrated VM**: `cmd/containerd-shim-katamaran-adopted-v2/main.go` daemonizes on containerd's `start` command, serves both the v3 and v2 TTRPC TaskService APIs on the shim socket, resolves the sandbox id from the pod's `katamaran.io/adopted-sandbox-id` annotation, locates the QEMU pid in the surviving cgroup, and implements Create/Start/State/Wait/Kill/Delete/Connect/Pids. Exit detection polls `/proc/<pid>` instead of pidfd (no CGO / kernel-version requirement); Delete removes the surviving cgroup; exec/checkpoint are unsupported by design and Pause/Resume are no-ops (QMP stop/cont not wired). `config/crd/runtimeclass-adopted.yaml` provides the matching RuntimeClass. Remaining work: live end-to-end verification and QMP `cont` if the post-migrate VM is paused. Treat `runtimeClassName: katamaran-adopted` as experimental until then.
...
3. **End state identical to A.** Pod name + UID still change (k8s pod names are immutable; can't truly transfer identity). The "seamlessness" is real but small: it only matters during the ~1s window between adoption-create and source-delete.
