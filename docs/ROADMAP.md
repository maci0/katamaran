...
2. **Kata sandbox adoption** — Make the migrated QEMU process appear as a regular Kata pod on the destination, visible to `kubectl`, manageable by Deployments, monitored by kubelet.
   ...
   **Paths forward**:
   ...
   - **Approach E (process re-parenting): in progress.** Step 1 (cgroup re-parent so the migrated QEMU survives the dest job's container exit) is **landed**: `internal/migration/dest.go::surviveContainerExit` writes the QEMU pid + its KVM helper kernel threads (`kvm-nx-lpage-recovery-<qemupid>`) into `/sys/fs/cgroup/katamaran-adopted/<sandbox-id>/cgroup.procs`. Live-verified end-to-end: 9m20s hang → 35s clean success. The QEMU now outlives the dest job container and is reachable on its QMP socket from any process on the dest node. Step 2 (custom containerd v2 shim that adopts the surviving QEMU into a new pod's lifecycle) is **scaffolding-only**: see `cmd/containerd-shim-katamaran-adopted-v2/main.go` (logs the invocation envelope on each containerd call, exits non-zero so the matching pod fails fast rather than silently sitting in `ContainerCreating`) + `config/crd/runtimeclass-adopted.yaml` (RuntimeClass `katamaran-adopted` whose handler resolves to the shim binary). The remaining work is the ttrpc TaskService implementation: Create/Start/Wait/State/Delete RPCs, pidfd-based wait on the adopted QEMU, QMP `cont` if the post-migrate VM is paused, cgroup teardown on Delete. Until that lands, do not flip adoption pods to `runtimeClassName: katamaran-adopted` on production — the scaffolding shim refuses to start.
