...
set +e
minikube -p "${PROFILE}" ssh -n "${PROFILE}" -- "sudo /usr/local/bin/katamaran -mode source -qmp '${SRC_SOCK}' -dest-ip '${NODE2_IP}' -vm-ip '${SRC_IP}' -shared-storage" 2>&1 | tee /tmp/katamaran-source.log
