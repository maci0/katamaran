...
func TestNativeDiscoverer_ListKataPods(t *testing.T) {
	...
	got, err := (&nativeDiscoverer{client: cs}).ListKataPods(context.Background())
	if err != nil {
		t.Fatal(err)
	}
	if len(got) != 2 {
...
func TestNativeDiscoverer_ListKataNodes(t *testing.T) {
	...
	got, err := (&nativeDiscoverer{client: cs}).ListKataNodes(context.Background())
	if err != nil {
		t.Fatal(err)
	}
